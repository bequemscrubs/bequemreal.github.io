...
async function loadPendingReviews() {
    ...
    // Not present on reviews.html, only on the admin page.
    ...
    if (!client) {
        container.innerHTML = "<p>Unable to load pending reviews.</p>";
        return;
    }
    ...
    container.querySelectorAll(".approve-btn").forEach(function (button) {
        button.addEventListener("click", function () {
            approveReview(this.dataset.id);
        });
    });

    container.querySelectorAll(".reject-btn").forEach(function (button) {
        button.addEventListener("click", function () {
            rejectReview(this.dataset.id);
        });
    });
}
